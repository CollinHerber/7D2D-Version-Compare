...
Desktop tool for comparing 7 Days to Die XML and ItemIcons files between game versions and spotting mod conflicts caused by XML changes.

![7D2D Version Compare preview](docs/preview.gif)

## What It Does

- Compares XML files and ItemIcons assets between two local 7 Days to Die version folders.
- Lists changed files by added, removed, and modified status.
...
- Hides whitespace-only XML changes by default while keeping the option toggleable.
- Starts in a side-by-side diff view with added and removed lines highlighted, with a toggle for the inline diff layout.
- Opens each file at the first changed area and lets you jump between changed areas.
- Marks ItemIcons asset changes with an ICON badge so they are easy to distinguish from XML text diffs.
- Optionally compares a mod folder against changed game XML and flags conflicts at the XPath target level.
...
The app does not include 7 Days to Die files. Add your own XML snapshots and optional `ItemIcons` folders under the `Versions` folder, using one folder per game version.
...
Every direct child folder under `Mods` appears in the Mod Compare dropdown. When you select a mod, the app checks whether any mod XML files map to XML files changed between the selected game versions. ItemIcons files are included in version comparisons, but they do not participate in XPath mod conflict detection.
...
The app remembers the selected versions, selected mod, folder/conflict filters, whitespace toggle, and diff layout on the next launch.
...
Selected UI settings are saved on your machine at:

```text
%LOCALAPPDATA%/7D2D-Version-Compare/settings.json
```

If a saved version or mod folder no longer exists, the app falls back to the first available version pair and clears the unavailable mod selection.
...
The cache is refreshed automatically when either selected version folder changes. The app checks comparable file paths, file counts, total bytes, latest write time, and file metadata before reusing a cached diff.
...
Versions/<version name>/**/ItemIcons/**/*
```
...
If a comparison looks stale after manually editing files, restart the app or delete the local diff cache folder. The app should normally detect changed XML and ItemIcons metadata and refresh automatically.
